...
{
    public class UpdateActivityDomainSucceededHandlerTests
    {
        private readonly Mock<IUnitOfWork> _unitOfWorkMock;
        private readonly Mock<IContextAccessor> _contextAccessorMock;
        private readonly Mock<IMapper> _mapperMock;

        public UpdateActivityDomainSucceededHandlerTests()
        {
            ...
            _mapperMock.Setup(x => x.Map<UpdateActivitySucceeded>(It.IsAny<UpdateActivityDomainSucceeded>()))
                .Returns(new UpdateActivitySucceeded(Guid.NewGuid(), Guid.NewGuid(), Guid.NewGuid(), Guid.NewGuid(),
                    "test", "completed", DateTime.Now, "test/test"));
        }

        [Fact]
        public async Task UpdateActivityDomainSucceeded_SaveToOutbox_Succeeds()
        {
            // Arrange
            var activity = new Activity(
                ...
                "test/test",
                DateTime.Now);
            var notification = new UpdateActivityDomainSucceeded(activity);
            var handler = new UpdateActivityDomainSucceededHandler(_unitOfWorkMock.Object, _contextAccessorMock.Object, _mapperMock.Object);

            // Act
            await handler.Handle(notification, default);

            // Assert
            _mapperMock.Verify(x => x.Map<UpdateActivitySucceeded>(It.IsAny<UpdateActivityDomainSucceeded>()), Times.Once);
